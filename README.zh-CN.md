...
- 🎨 **六套主题**：浅色 / 深色（对齐 DSH 中性灰）/ 深海 / 暗紫 / 墨绿，另有 `auto` 模式 0.5 秒内跟随 DSH —— 一次选择，看板、工作台、simple 页全部生效

零依赖 —— 只用 Python 标准库 + 原生 HTML/JS，拷走即用。

想了解全貌？见 [docs/ARCHITECTURE.zh-CN.md](docs/ARCHITECTURE.zh-CN.md)。
...
> 图中为真实运行案例：5660 原子聚合物前驱体体系 300K 恒温松弛（共 200 万步），当前约 75,000 步、1730 K、风险评分 0、相位机处于 MONITOR 观测态。
